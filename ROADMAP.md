...
- [x] **Agent Identity Continuity Signals** — **SHIPPED 2026-05-05**: Registration accepts optional model identity fields (`llm_provider`, `llm_service`, `model_name`, `model_uuid`) and emits a generic `identity_continuity_event` task when a previously seen agent name reappears with changed model/provider identity. The payload carries prior/new model IDs, `continuity_check_pending`, and `snapshot_ref` without importing Harkonnen, Calvin, or Labrador concepts. Regression test: `ipc::tests::identity_continuity_payload_records_prior_and_new_model_identity`.
