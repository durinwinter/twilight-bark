...
pub struct IpcServer {
    ...
    /// task_id → TaskRecord — tracks every in-flight task for routing replies
    /// and triggering timeouts.
    pub pending_tasks: Arc<DashMap<String, TaskRecord>>,
}
...
impl IpcServer {
    ...
    async fn dispatch(&self, req: IpcRequest, registered: &mut Option<String>) -> IpcResponse {
        match req {
            ...
            IpcRequest::ListTasks => {
                let now = Instant::now();
                let tasks: Vec<serde_json::Value> = self.pending_tasks.iter().map(|e| {
                    let elapsed = now.duration_since(e.value().created_at).as_millis() as u64;
                    json!({
                        "task_id":     e.key(),
                        "source_uuid": e.value().source_uuid,
                ...
                IpcResponse {
                    ok: true,
                    tasks: Some(serde_json::Value::Array(tasks)),
                    ..Default::default()
                }
            }
            ...
            IpcRequest::ReplyTask { task_id, output_json, success } => {
                ...
                if let Some((_, record)) = self.pending_tasks.remove(task_id.as_str()) {
                    let event = json!({
                        "event":       "task_result",
                        "task_id":     &task_id,
                    ...
                    self.push_task_event(&record.source_uuid, event);
                }

                // Publish to Zenoh for bus-wide visibility (Sankey, eventlog, etc.)
